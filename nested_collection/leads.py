...
all_sources= {di.get("source") for di in leads }

print(all_sources)


# source summary
all_source_list=[di.get("source") for di in leads]

source_count = {s:all_source_list.count(s) for s in all_source_list}
...
conversion_rate = status_count.get("Qualified")/ sum(status_count.values())

print(conversion_rate)

"""
google_Ad_conversion_rate = total_number_of_converted_googleAd/ total_number_googleAd

"""

google_Ad_status=[ di.get("status") for di in leads if di.get("source")=="Google Ads"]

google_status_summary = {s:google_Ad_status.count(s) for s in google_Ad_status}

print(google_status_summary)

google_Ad_Conversion_rate = google_status_summary.get("Converted",0) / sum(google_status_summary.values())

print("Google conversion rate: ",google_Ad_Conversion_rate)

# course:no of leads

# course
all_courses =  [di.get("course") for di in leads]

course_count = {c:all_courses.count(c) for c in all_courses}
