...
passenger_class= {di.get("class") for di in titanic_data}
...
survived_childs = [di for di in titanic_data if di.get("survived")==1 and di.get("age")<13]

print(f"survived childern count{len(survived_childs)}")

# q5:name whose fare > 30

fare_filter = [di.get("name") for di in titanic_data if di.get("fare")>30]
print("fare filter ",fare_filter)
...
print("female survival rate",len(survived_female)/len(female_passengers)*100)
